...
DROP TABLE IF EXISTS `db_vmd`.`tb_veh_part`;
CREATE TABLE `db_vmd`.`tb_veh_part`
(
    `id`              BIGINT       NOT NULL AUTO_INCREMENT COMMENT '主键',
    `sn`              VARCHAR(255) NOT NULL COMMENT '零部件序列号',
    `no`              VARCHAR(255)          DEFAULT NULL COMMENT '零部件编号',
    `ecu`             VARCHAR(255) NOT NULL COMMENT '零部件ECU',
    `ecu_config_word` VARCHAR(255)          DEFAULT NULL COMMENT 'ECU配置字',
  ...
  DEFAULT CHARSET = utf8mb4 COMMENT ='车辆零部件表';